...
const PUBLIC_PREFIXES = ["/reset-password/", "/verify-email/", "/api/auth/", "/_next/", "/favicon.ico", "/announcements", "/staff", "/programs", "/requests", "/facilities", "/uploads"];
...
export async function proxy(req: NextRequest) {
  ...
  if (
    PUBLIC_PREFIXES.some(
      (p) => pathname === p || (p.endsWith("/") ? pathname.startsWith(p) : pathname.startsWith(p + "/"))
    )
  ) {
